	...
	tests := []struct {
		...
		},
		{
			...
			verify: func(runData workflow.RunData) error {
				data, ok := runData.(*TestInitData)
				if !ok {
					return fmt.Errorf("invalid type assertion on run data")
				}
	...
	tests := []struct {
		...
		},
		{
			...
			prep: func(rd workflow.RunData) error {
				...
				if !ok {
					return fmt.Errorf("type assertion failed for workflow.RunData on TestInitData")
				}
				return createPods(data.RemoteClient(), namespace, util.KarmadaMetricsAdapterName(name), replicas, karmadaMetricAdapterLabels)
			},
			verify: func(runData workflow.RunData) error {
				data, ok := runData.(*TestInitData)
				if !ok {
