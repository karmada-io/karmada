...
func TestRunUploadKubeconfig(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "RunUploadKubeconfig_InvalidTypeAssertion_TypeAssertionFailed",
			runData: &MyTestData{Data: "test"},
			wantErr: true,
			errMsg:  "upload-config task invoked with an invalid data struct",
		},
		{
			name: "RunUploadKubeconfig_ValidTypeAssertion_TypeAssertionSucceeded",
...
func TestRunUploadAdminKubeconfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "RunUploadAdminKubeconfig_InvalidTypeAssertion_TypeAssertionFailed",
			runData: &MyTestData{Data: "test"},
		...
		{
			name: "RunUploadAdminKubeconfig_WithKarmadaAPIServerClusterIPServiceType_SecretCreated",
			runData: &TestInitData{
				Name:      name,
				Namespace: namespace,
		...
		{
			name: "RunUploadAdminKubeconfig_WithKarmadaAPIServerNodePortServiceType_SecretCreated",
			runData: &TestInitData{
				Name:      name,
				Namespace: namespace,
				ComponentsUnits: &operatorv1alpha1.KarmadaComponents{
...
func verifySecret(client *fakeclientset.Clientset, name, namespace string, expectedEndpoint string) error {
	secretName := util.AdminKubeconfigSecretName(name)
