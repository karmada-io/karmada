...
func TestNewInitJob(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := containAllTasks(initJob.Tasks, test.tasksExpected)
			if err != nil {
	...
	tests := []struct {
		...
		},
		{
			...
			mockFunc: func() {
				...
				buildClientFromSecretRefFactory = func(clientset.Interface, *operatorv1alpha1.LocalSecretReference) (clientset.Interface, error) {
