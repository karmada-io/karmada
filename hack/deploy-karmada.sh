...
function generate_cert_related_secrets {
    ...
    # Generate different types of Secrets based on component requirements
    ...
    generate_server_client_etcd_secret "karmada-aggregated-apiserver" "${karmada_ca}" \
        ...
        "${KARMADA_AGGREGATED_APISERVER_CLIENT_CRT}" "${KARMADA_AGGREGATED_APISERVER_CLIENT_KEY}" \
        "${KARMADA_AGGREGATED_APISERVER_ETCD_CLIENT_CRT}" "${KARMADA_AGGREGATED_APISERVER_ETCD_CLIENT_KEY}"
    generate_server_client_etcd_secret "karmada-search" "${karmada_ca}" \
        "${KARMADA_SEARCH_SERVER_CRT}" "${KARMADA_SEARCH_SERVER_KEY}" \
        "${KARMADA_SEARCH_CLIENT_CRT}" "${KARMADA_SEARCH_CLIENT_KEY}" \
        "${KARMADA_SEARCH_ETCD_CLIENT_CRT}" "${KARMADA_SEARCH_ETCD_CLIENT_KEY}"
    generate_server_client_secret "karmada-webhook" "${karmada_ca}" \
        "${KARMADA_WEBHOOK_SERVER_CRT}" "${KARMADA_WEBHOOK_SERVER_KEY}" \
        "${KARMADA_WEBHOOK_CLIENT_CRT}" "${KARMADA_WEBHOOK_CLIENT_KEY}"
    ...
    generate_server_client_secret "karmada-metrics-adapter" "${karmada_ca}" \
        ...
        "${KARMADA_METRICS_ADAPTER_CLIENT_CRT}" "${KARMADA_METRICS_ADAPTER_CLIENT_KEY}"
    ...
    generate_server_client_secret "karmada-scheduler-estimator" "${karmada_ca}" \
        ...
        "${KARMADA_SCHEDULER_ESTIMATOR_CLIENT_CRT}" "${KARMADA_SCHEDULER_ESTIMATOR_CLIENT_KEY}"
    generate_client_secret "karmada-controller-manager" "${karmada_ca}" \
        "${KARMADA_CONTROLLER_MANAGER_CLIENT_CRT}" "${KARMADA_CONTROLLER_MANAGER_CLIENT_KEY}"
    generate_client_grpc_secret "karmada-scheduler" "${karmada_ca}" \
        "${KARMADA_SCHEDULER_GRPC_CRT}" "${KARMADA_SCHEDULER_GRPC_KEY}" \
        "${KARMADA_SCHEDULER_CLIENT_CRT}" "${KARMADA_SCHEDULER_CLIENT_KEY}"

    generate_client_grpc_secret "karmada-descheduler" "${karmada_ca}" \
        "${KARMADA_DESCHEDULER_GRPC_CRT}" "${KARMADA_DESCHEDULER_GRPC_KEY}" \
        "${KARMADA_DESCHEDULER_CLIENT_CRT}" "${KARMADA_DESCHEDULER_CLIENT_KEY}"
...
# Generate Secret for Client-only components
# Includes: ca.crt, client.crt, client.key
function generate_client_secret() {
    ...
    cp "${REPO_ROOT}/artifacts/secret/karmada-client-secret.yaml" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${component}/${component}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${ca_crt}/${ca_crt}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${client_crt}/${client_crt}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${client_key}/${client_key}/g" "${TEMP_PATH}/${component}-secret.yaml"
...
# Generate Secret for GRPC Client components
# Includes: ca.crt, grpc.crt, grpc.key, client.crt, client.key
function generate_client_grpc_secret() {
    local component=$1
    local ca_crt=$2
    local grpc_crt=$3
    local grpc_key=$4
    ...
    cp "${REPO_ROOT}/artifacts/secret/karmada-client-grpc-secret.yaml" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${component}/${component}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${ca_crt}/${ca_crt}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${grpc_crt}/${grpc_crt}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${grpc_key}/${grpc_key}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${client_crt}/${client_crt}/g" "${TEMP_PATH}/${component}-secret.yaml"
    sed -i'' -e "s/\${client_key}/${client_key}/g" "${TEMP_PATH}/${component}-secret.yaml"
