...
KARMADA_VERSION=$(curl -sl https://api.github.com/repos/karmada-io/karmada/releases |grep tag_name | awk '{print $2}'| sed -n '1p'| awk -F '-' '{print $1}'| tr -d 'v"')
...
function usage() {
  ...
  echo "Example: hack/deploy-karmada.sh ~/.kube/config karmada-host true"
...
if [[ -n "$4" ]]; then
	...
	kubectl exec -it $name -nkarmada-system -- mkdir -p /var/lib/karmada/"${KARMADA_VERSION}"
...
kubectl wait --for=condition=Ready --timeout=400s karmada karmada-demo -n test
