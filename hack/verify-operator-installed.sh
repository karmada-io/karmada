...
# This script starts a local karmada control plane based on current codebase via karmada-operator and with a certain number of clusters joined.
...
function usage() {
    echo "Usage:"
    echo "    hack/verify-operator-installed.sh [HOST_IPADDRESS] [-h]"
    echo "Args:"
...
make image-karmada-agent GOOS="linux" --directory="${REPO_ROOT}"
make image-karmada-scheduler-estimator GOOS="linux" --directory="${REPO_ROOT}"

GO111MODULE=on go install "github.com/karmada-io/karmada/cmd/karmadactl"
GOPATH=$(go env GOPATH | awk -F ':' '{print $1}')
KARMADACTL_BIN="${GOPATH}/bin/karmadactl"

#step3. wait until the host cluster and member cluster ready
...
#step4. prepare the newest crds
echo "Prepare the newest crds"
make image-karmada-operator GOOS="linux" --directory=.
cd  charts/karmada/
tar -zcvf ../../crds.tar.gz _crds
cd -

#step5. install karmada control plane components via karmada-operator
"${REPO_ROOT}"/hack/deploy-karmada-by-operator.sh "${MAIN_KUBECONFIG}" "${HOST_CLUSTER_NAME}" "true" "crds.tar.gz"
