...
# This script holds binary build related functions.
...
get_ldflags() {
  ...
  function add_ldflag() {
    ...
    ldflags+=(
      "-X ${KARMADA_GO_PACKAGE}/pkg/version.${key}=${val}"
    )
  }
...
function build_binaries() {
  for binArg in "$@"; do
    local name="karmada-${binArg}"
    local path="${KARMADA_GO_PACKAGE}/cmd/${binArg}"
    if [[ ${binArg} == "ctl" ]]; then
      name="karmadactl"
      path="${KARMADA_GO_PACKAGE}/cmd/karmadactl"
    fi
