	...
	ginkgo.Context("FlinkDeployment scheduling", func() {
		...
		ginkgo.It("base case: propagate FlinkDeployment to one cluster", func() {
			...
			ginkgo.By("verify ResourceBinding components field", func() {
				var jobManagerComponent, taskManagerComponent *workv1alpha2.Component
				...
				gomega.Eventually(func(g gomega.Gomega) {
					...
					g.Expect(resourceBinding.Spec.Components).ShouldNot(gomega.BeNil())
					g.Expect(resourceBinding.Spec.Components).Should(gomega.HaveLen(2), "components should have exactly 2 items")
					...
					g.Expect(jobManagerComponent).ShouldNot(gomega.BeNil(), "jobmanager component should exist")
					g.Expect(taskManagerComponent).ShouldNot(gomega.BeNil(), "taskmanager component should exist")
