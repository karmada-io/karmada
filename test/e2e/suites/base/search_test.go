...
var _ = ginkgo.Describe("[karmada-search] karmada search testing", ginkgo.Ordered, func() {
	...
	ginkgo.Describe("karmada proxy testing", ginkgo.Ordered, func() {
		...
		ginkgo.Describe("access resources testings", func() {
			...
			ginkgo.Context(fmt.Sprintf("add two resourceRegistry for caching secret for cluster %s ", member1), func() {
				...
				ginkgo.BeforeAll(func() {
					...
					m1Secret2 = testhelper.NewSecret(testNamespace2, m1SecretName2, map[string][]byte{"user": []byte("karmada")})
				...
				ginkgo.AfterAll(func() {
					...
					framework.RemoveSecret(member1Client, testNamespace2, m1SecretName2)
					framework.RemoveResourceRegistry(karmadaClient, rrName)
					framework.RemoveResourceRegistry(karmadaClient, rr2Name)
				})

				ginkgo.It("cloud list secret", func() {
					...
					fromM1TestNamespace2 := framework.GetResourceNames(m1Dynamic.Resource(secretGVR).Namespace(testNamespace2))
					ginkgo.By(fmt.Sprintf("list secret from cluster %s namespace %s: %s", member1, testNamespace2, strings.Join(sets.List(fromM1TestNamespace2), ",")))
					...
					gomega.Eventually(func(g gomega.Gomega) {
						...
						proxyList, err = proxyClient.CoreV1().Secrets(testNamespace2).List(context.TODO(), metav1.ListOptions{})
						g.Expect(err).ShouldNot(gomega.HaveOccurred())
						for _, item := range proxyList.Items {
							fromProxy.Insert(item.Name)
						}
