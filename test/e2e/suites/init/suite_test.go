...
const (
	defaultRegistry = "docker.io/karmada"
...
func init() {
	...
	flag.StringVar(&hostContext, "host-context", "karmada-init", "Name of the host cluster context in control plane kubeconfig file.")
}
...
var _ = ginkgo.SynchronizedBeforeSuite(func() []byte { return nil }, func([]byte) {
	...
	karmadaAggregatedAPIServerImage = fmt.Sprintf("%s/karmada-aggregated-apiserver:%s", registry, version)
	...
	karmadaAgentImage = fmt.Sprintf("%s/karmada-agent:%s", registry, version)
