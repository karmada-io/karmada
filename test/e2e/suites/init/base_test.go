...
const (
	karmadactlTimeout = time.Second * 120

	// The register command creates the pull-mode karmada-agent Deployment and
	// waits for it to be ready. In CI this can include image pull and rollout
	// latency, so keep this aligned with the init command timeout.
	karmadactlRegisterTimeout = KarmadactlInitTimeOut
)
	...
	ginkgo.Context("Karmadactl init base testing", func() {
		...
		ginkgo.It("Deploy a karmada instance and do a propagation testing", func() {
			...
			ginkgo.By("register a pull mode cluster", func() {
				...
				cmd = framework.NewKarmadactlCommand(
					"", "", karmadactlPath, "", karmadactlRegisterTimeout, "register", karmadaAPIEndpoint, "--token", token,
					"--kubeconfig="+pullModeKubeConfigPath,
					"--cluster-name", pullModeClusterName, "--context", pullModeClusterName, "--karmada-agent-image", karmadaAgentImage,
