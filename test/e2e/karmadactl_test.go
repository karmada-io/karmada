...
var _ = ginkgo.Describe("Karmadactl get testing", func() {
	...
	ginkgo.Context("Test cluster scope switching", func() {
		var deployment *appsv1.Deployment
		...
		ginkgo.It("should switch cluster scope successfully", func() {
			...
			ginkgo.By("should display the deployment in Karmada control plane and member1", func() {
				cmd := framework.NewKarmadactlCommand(kubeconfig, karmadaContext, karmadactlPath, deployment.Namespace, karmadactlTimeout, "get", "deployment", deployment.Name, "-C", member1)
				output, err := cmd.ExecOrDie()
				gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
				gomega.Expect(strings.Contains(output, "Karmada")).Should(gomega.BeTrue())
			...
			ginkgo.By("should display the deployment in Karmada control plane and all member clusters", func() {
				cmd := framework.NewKarmadactlCommand(kubeconfig, karmadaContext, karmadactlPath, deployment.Namespace, karmadactlTimeout, "get", "deployment", deployment.Name, "--all-clusters")
				output, err := cmd.ExecOrDie()
				gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
				gomega.Expect(strings.Contains(output, "Karmada")).Should(gomega.BeTrue())
				gomega.Expect(strings.Contains(output, member1)).Should(gomega.BeTrue())
			...
			ginkgo.By("should ignore resources in Karmada control plane", func() {
				cmd := framework.NewKarmadactlCommand(kubeconfig, karmadaContext, karmadactlPath, deployment.Namespace, karmadactlTimeout, "get", "deployment", deployment.Name, "--ignore-karmada", "--all-clusters")
				output, err := cmd.ExecOrDie()
				gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
				gomega.Expect(strings.Contains(output, "Karmada")).ShouldNot(gomega.BeTrue())
			})

			ginkgo.By("should return a prompt when no target cluster is specified", func() {
				cmd := framework.NewKarmadactlCommand(kubeconfig, karmadaContext, karmadactlPath, deployment.Namespace, karmadactlTimeout, "get", "deployment", deployment.Name, "--ignore-karmada")
				_, err := cmd.ExecOrDie()
				gomega.Expect(err).Should(gomega.HaveOccurred())
				gomega.Expect(strings.Contains(err.Error(), "please specify at least one target cluster")).Should(gomega.BeTrue())
			})
		})
	})
	...
	ginkgo.Context("Test karmadactl get for existing resource", func() {
		...
		ginkgo.It("should get the existing pod successfully", func() {
			cmd := framework.NewKarmadactlCommand(kubeconfig, karmadaContext, karmadactlPath, namespace, karmadactlTimeout, "get", "pods", podName, "-C", member1, "--ignore-karmada")
			output, err := cmd.ExecOrDie()
			gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
