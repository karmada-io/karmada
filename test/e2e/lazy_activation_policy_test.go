...
var _ = ginkgo.Describe("Lazy activation policy testing", func() {
	...
	var originalCluster, modifiedClutser string
	...
	ginkgo.BeforeEach(func() {
		...
		modifiedClutser = framework.ClusterNames()[1]
	...
	ginkgo.Context("1. Policy created before resource", func() {
		ginkgo.JustBeforeEach(func() {
			...
			waiPropagatePolicyExistInKarmada(namespace, policyName)
		...
		ginkgo.Context("Propagate dependencies", func() {
			...
			ginkgo.It("Combined Case 5 (Propagate dependencies)", func() {
				...
				ginkgo.By("step 2: change of lazy policy will not take effect", func() {
					changePlacementTargetCluster(policy, modifiedClutser)
				...
				ginkgo.By("step 3: lazy policy take effect when deployment updated, dependencies can also been propagated.", func() {
					...
					waitDeploymentPresentOnCluster(modifiedClutser, namespace, deploymentName)
					waitConfigMapPresentOnCluster(modifiedClutser, namespace, deploymentName)
				})
			})
		})
	})
	...
	ginkgo.Context("2. Policy created after resource", func() {
		ginkgo.JustBeforeEach(func() {
			framework.CreateDeployment(kubeClient, deployment)
			waitDeploymentExistInKarmada(namespace, deploymentName)
			framework.CreatePropagationPolicy(karmadaClient, policy)
...
// waitDeploymentExistInKarmada wait deployment exist in karmada
func waitDeploymentExistInKarmada(namespace, name string) {
	framework.WaitDeploymentFitWith(kubeClient, namespace, name, func(_ *appsv1.Deployment) bool {
		return true
	})
}

// waiPropagatePolicyExistInKarmada wait PropagatePolicy exist in karmada
func waiPropagatePolicyExistInKarmada(namespace, name string) {
