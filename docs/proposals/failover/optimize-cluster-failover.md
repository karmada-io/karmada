...
In all currently released versions of Karmada, when the Failover function is enabled, the behavior is as follows: when the system detects an abnormal cluster health status condition (the cluster health status monitoring takes debounce into account, and the status condition will only be modified if the abnormality or normality persists within a sliding time window), it will automatically set the status condition of the cluster object to "NotReady". Then, the system will automatically add a taint with the effect of NoExecute to the target cluster object. This taint will trigger the migration of all resources on the target cluster, including non-workload resources (such as Configmap, Secret), workload resources (such as Deployment, StatefulSet), and CRD resources, etc. After 5 minutes (this time is a configurable default value, which is the taint toleration time and is added by the system by default when the propagation policy is created. If the user configures the corresponding toleration, it will override this configuration).

Based on the collected user feedback, the following aspects of the Failover function need improvement:

1. In some versions[1], the Failover function is enabled by default. However, users lack a full understanding and proper anticipation management of the impacts of the migration behavior. As a result, they can only disable the function when unprepared, which prevents the full utilization of the Failover capability.
2. The Failover function lacks a mechanism that allows users to configure a large number of workloads to not migrate by default and only a small number of workloads to migrate.
3. There are corner cases in the implementation of the `gracefulEvictionTask` for the eviction results of cluster failure migration. This causes the elements in the [gracefulEvictionTask slice](https://github.com/karmada-io/karmada/blob/678362ba9ae3e02737e421b21a363a3b95be3b94/pkg/apis/work/v1alpha2/binding_types.go#L111) of the binding object to fail to be cleared, and this issue cannot be fixed with a simple patch.
4. The condition judgment for cluster failures is relatively simplistic and cannot be customized by users (Once the Failover function is enabled, a cluster will definitely be tainted with `NoExecute` when a specific condition occurs).
5. The Failover function fails to consider the overall health status of member clusters in Karmada instance and lacks migration degradation measures, posing a risk of an avalanche effect. For example, when 50% of the member clusters fail, the cluster failure migration in Karmada will not slow down or even stop.
...
- If a user actively adds a `NoExecute` taint to a cluster, it will still trigger the migration of resources on that cluster.
- During the process of a user removing a cluster, the system will add a `NoExecute` taint with the key `cluster.karmada.io/terminating` to the cluster, thus triggering the migration of resources on the cluster.
...
**Goal two:** Enable users to define "how to respond to cluster failover", that is, the migration strategy of workloads after a cluster failure occurs, such as the migration method and toleration time, at the application granularity.

**Goal three:** Modify the condition judgment mechanism for cluster failures to support user-defined settings.
...
- At the cluster failure condition judgment level: Which cluster status conditions will mark a cluster as failed, thus triggering the failure migration of resources on the cluster.
...
As a conservative and cautious user, a large number of services adopt a fixed redundant deployment scheme, and I don't want any non-human-initiated migrations to occur in the event of a cluster failure. A small number of services support a dynamic HA deployment scheme, and I hope that when a cluster failure occurs, the affected parts of these services can be migrated to a new healthy cluster.
...
2. Reconsider the types of taints and adjust them into the following categories according to the severity of cluster failures and the scope of affected applications:

- Impact new workloads' scheduling and running: NoSchedule. New workloads will not be scheduled, and will not trigger resource migration.
- Impact all workloads' scheduling and running: NoExecute (Reserve semantics: Hard migration, requiring users to add relevant comments to remind them of the impact in the API definition). New workloads will not be scheduled, and existing workloads will be migrated.
- Add a new type of taint between NoSchedule and NoExecute: PreferNoExecute.
...
- Reinstate: Hard migration for taints of type NoExecute.
    - Migration method: graceful or immediately (need to be decided).
- New addition: Respond to PreferNoExecute
- The failure migration strategy at the application granularity needs to include the following elements: :
    - Migration method: Graciously, Immediately
    - Toleration time

4. Eviction queue (The queue maintained in memory)

- It is necessary to consider the overall health of the system and have the ability to downgrade eviction behavior and slow down queue processing.
...
In light of the above design principles, it has been sorted out that the following modules in the current system do not meet the objectives of this proposal:
...
3. There is no application-level failure migration strategy provided.
...
This proposal adds a new API: TaintClusterByCondition, which can be used to describe what taints should be added to the target cluster when certain cluster status conditions are met. The relevant logic can be implemented by adding a new controller. This controller is responsible for monitoring changes in the cluster status conditions, that is, the `.status.conditions` field of the Cluster object, and then deciding how to modify the taints of the Cluster object according to the TaintClusterByCondition API defined by the user. When the matching conditions are no longer met, the taints added to the target cluster will be removed.

The TaintClusterByCondition resource is a Cluster scope resource. Its API group is `policy.karmada.io` and the API version is `v1alpha1`.

```go
type TaintClusterByCondition struct {
    metav1.TypeMeta   `json:",inline"`
    metav1.ObjectMeta `json:"metadata,omitempty"`

    // Spec represents the desired behavior of TaintConvert.
    // +required
    Spec TaintClusterByConditionSpec `json:"spec"`
}

type TaintClusterByConditionSpec struct {
    // TargetClusters specifies the clusters that TaintClusterByCondition needs to pay attention to.
    // For clusters that meet the DecisionConditions, TaintsToAdd will be added.
    // +optional
    TargetClusters *TargetClusters `json:"targetClusters,omitempty"`

    // DecisionMatches indicates the decision matches of triggering
    // the taint system to add taints on the cluster object.
    // As long as any one DecisionMatch matches, the TaintsToAdd will be added.
...
type TargetClusters struct {
    // ClusterNames are the list of clusters to be selected.
...
type TaintToAdd struct {
    ...
    // If AddAfterSeconds is not set, the taint is added immediately.
    // +optional
    AddAfterSeconds *int64 `json:"addAfterSeconds,omitempty"`
}
```

Let's take an example to explain the processing logic of "taint cluster by Conditions", the user creates the following TaintClusterByCondition resource:

```yaml
apiVersion: policy.karmada.io/v1alpha1
kind: TaintClusterByCondition
metadata:
  name: taint-cluster-by-condition-sample
spec:
  targetCluster:
    clusterNames:
    - cluster1
...
When the controller watches to the change of the cluster status, it will judge the cluster status condition, match the `taint-cluster-by-condition-sample` TaintClusterByCondition resource, and add the corresponding taint to the cluster member1 after 5 minutes:
...
At this point, cluster member1 is tainted with PreferNoExecute with key `cluster.karmada.io/not-ready`.

### Design of Taint Types

At present, there are two kinds of taints used in the Karmada system: NoSchedule and NoExecute. NoSchedule affects the scheduling and operation of new workloads. When there is a NoSchedule taint on the cluster, the newly created workloads of the system will not be scheduled to the cluster, which ensures that the resources on the cluster will not be migrated. NoExecute affects the scheduling and operation of all workloads. It is a hard migration. When there is a NoExecute taint on the cluster, not only will the newly created workloads not be scheduled to the cluster, but also the existing workloads on the cluster will be migrated.

The statistics of NoSchedule and NoExecute blemishes involved in the current Karmada system are as follows:
...
> Note: In this proposal, these taints will no longer be automatically added by the system, but need to be defined through the "taint cluster by Conditions" rule.

We plan to introduce a new type of taint: PreferNoExecute, which has an effect between NoSchedule and NoExecute and only affects the scheduling and operation of some workloads. Moreover, when there is a taint of PreferNoExecute on the cluster, tolerations do not need to be considered, and the resources on the cluster are not migrated by default, which is obviously different from NoSchedule and NoExecute taints; in this case, the active migration strategy of the taint is configured through `PP/CPP.spec.failover.cluster`, which is passed through to the RB/CRB object.
...
We plan to introduce an application-level cluster failover strategy to configure an active migration policy for PreferNoExecute taint.

Introducing new fields in PropagationPolicy/ClusterPropagationPolicy `.spec.failover` API:
...
const (
  // Immediately indicates resource will be migrated immediately after the tolerated time has exceeded.
  Immediately PurgeMode = "Immediately"
  // Graciously indicates that after exceeding the tolerance time, the resource will be migrated in an graceful manner.
...
spec:
  ...
  failover:
    cluster:
      pureMode: Immediately
      tolerationSeconds: 100
```

After the user applies this PropagationPolicy to Karmada to propagate the deployment nginx resource, when the member cluster member1 is tainted with PreferNoExecute by the system, the deployment nginx propagated to the member1 cluster will be evicted after 100 seconds.

If the user does not configure the `.spec.failover.cluster` field in the PropagationPolicy, the resources matched by this PropagationPolicy will not trigger a migration after the cluster is tainted with PreferNoExecute.

Here, we would like to emphasize the difference between `pp/cpp.spec.placement.clusterTolerations` and `pp/cpp.spec.failover.cluster,tolerationSeconds`. For the former, it only takes effect for taints with the effect of NoSchedule and NoExecute; while for the latter, it only takes effect for taints with the effect of PreferNoExecute.

> Note that we also need to state this effective rule in the API of PP/CPP.

### Eviction Queue

The eviction queue described in this section refers to the fact that when the cluster is determined to be faulty (containing NoExecute or PreferNoExecute taints), the affected resources[2] on the cluster are added to the eviction queue, which is a queue in memory managed by the controller. Each failed cluster creates a separate eviction queue. The lifecycle of the queue begins with the cluster failure and ends with the cluster recovery. In this way, for a healthy cluster, there will be no corresponding eviction queue in the controller.

> Note[2]: How to determine the affected resources varies depending on the type of taint:
> For a NoExecute taint, the resource is an affected resource when the resource-related tolerance policy cannot tolerate the taint or can only tolerate it for a period of time.
> For the PreferNoExecute taint, there is no need to consider the tolerance policy. By checking the RB/CRB object associated with the resource, it is determined whether the `.spec.failover.cluster` is configured, and if so, it indicates that the resource is an affected resource.

**1. How does the eviction queue control the rate of resource eviction, and how does it slow down when the cluster fails in a large area?**

By drawing on the control method of the node-controller in Kubernetes for the rate of adding taints to nodes, this proposal plans to introduce the following control parameters in the karmada-controller-manager component:
...
**2. If the resource is in the eviction queue, it may not be processed yet. At this time, the failed cluster has recovered. How to deal with it?**

When the faulty cluster recovers, the unprocessed elements in the eviction queue will be discarded, and then the queue will be destroyed. Therefore, the resources that have not been processed will avoid being migrated.

Here, a scenario needs to be explained. After a cluster fails, even if the user modifies the tolerance policy of the resources or the application failure migration policy, which makes the resources gain or lose the qualification for eviction, the final eviction behavior will not change accordingly. Therefore, users are required to define the policies in advance. There is one exception. If the controller component switches the primary or restarts, the eviction queue will be rebuilt.

**3. What metrics need to be provided for the eviction queue to facilitate users in defining problems and setting up alarms?**
...
- evict_success_rate: The ratio of the number of successfully processed resources to the total number of processed resources, with labels:
  - cluster_name: Identifies the name of the cluster to which the queue belongs, facilitating the differentiation of queues from different clusters.
- enque_processing_duration_seconds: The time consumed to process all resources on a cluster after a cluster failure, with labels:
  - cluster_name: Identifies the name of the cluster to which the queue belongs, facilitating the differentiation of queues from different clusters.
- enque_rate: The enqueue rate of resources on a faulty cluster, with labels:
  - cluster_name: Identifies the name of the cluster to which the queue belongs, facilitating the differentiation of queues from different clusters.
- enque_kind_rate: The enqueue rate of resources of different types on a faulty cluster, with labels:
  - cluster_name: Identifies the name of the cluster to which the queue belongs, facilitating the differentiation of queues from different clusters.
  - resource_kind: Marks the type of the enqueued resource, which is formed by concatenating the APIVersion and Kind of the resource.
- cluster_fault_num: The number of faulty clusters.
- cluster_fault_rate: The failure rate of clusters in a Karmada instance.

**4. Among the above metrics, there is an indicator called evict_success_rate. So, when will the resource eviction fail?**
...
Keep the Failover FeatureGate and deprecate the GracefulEviction FeatureGate.

The Failover FeatureGate will control the start and stop of the entire Failover function, including the judgment of cluster failure conditions, the enqueuing of resources into the eviction queue, and the execution of resource eviction. The capabilities of the latter two are built in the taint-manager. This means that even if users actively add NoExecute or PreferNoExecute taints to the cluster through a custom controller, no resource migration due to cluster failures will occur in the Karmada system.
...
The main reason for this problem is the second point mentioned above. Since the scheduling result is empty and does not meet the prerequisite condition, logically, it cannot enter the gracefulEvictionTask cleaning process of the graceful_eviction_controller. Moreover, without a scheduling retry, the gracefulEvictionTask will never be cleaned up. Even after the cluster recovers, the target resource cannot be rescheduled to this cluster, thus creating a logical deadlock.

To solve this problem, the following three strategies need to be adopted:

1. No longer distinguish between completed scheduling and successful scheduling, and set `binding.Status.schedulerObservedGeneration = binding.Generation` in both cases. However, this cannot rule out the situation of continuous scheduling failures. For example, for load distribution of the divided type, the scheduling fails due to insufficient cluster resources.
2. The `graceful-eviction-timeout` parameter of the controller is used to represent the maximum waiting time for task cleaning in the eviction queue. This time is calculated from the completion of the scheduling. It should be modified to be calculated from the time when the task is added to the gracefulEvictionTask.
3. During the scheduling process, it is allowed to schedule resources to a cluster that already exists in the gracefulEvictionTask. If a cluster exists in both the scheduling result and the gracefulEvictionTask at the same time, the graceful_eviction_controller is responsible for clearing the target elements in the gracefulEvictionTask (this can be achieved by adding an eviction plugin mentioned below. The logic of this plugin is to determine whether a certain cluster exists in both the scheduling result and the eviction queue. If so, pass).
...
2. Since the PreferNoExecute taint is newly added in this proposal, users do not have to make overly drastic changes to the resource migration strategy as they would with the NoExecute taint. In the scenario where users explicitly add the NoExecute taint, the Karmada system no longer needs to use the karmada-webhook component to add a default tolerance of 300 seconds for the NoExecute taint of the cluster to the PropagationPolicy/ClusterPropagationPolicy resources. The definition of the tolerance policy is entirely handled by the users.

3. The taint-manager controller is started together with the cluster-controller, instead of being started as a separate controller. As a result, it cannot be controlled through the `controllers` parameter of the controller. This aspect will be optimized and improved in this optimization.
...
When users use the `karmadactl unjoin` command, a NoExecute taint with the key `cluster.karmada.io/terminating` will be added to the cluster object. The purpose is to migrate the resources on this cluster to other clusters before deleting the cluster object, so as to gracefully remove the cluster. This change was introduced by [PR2373](https://github.com/karmada-io/karmada/pull/2373).

Before the current proposal, this behavior was controlled by the `enable-taint-manager` parameter of the controller. When the `enable-taint-manager` parameter was set to `false`, executing the `karmadactl unjoin` command would not trigger the migration of resources on the target cluster.

In the current proposal, the startup of the taint-manager is uniformly controlled by the `controllers` parameter, and the `enable-taint-manager` parameter will be deprecated. In addition, the Failover FeatureGate will also determine whether the taint-manager works. According to the principles we set for judging cluster failure conditions, it is inappropriate for the Karmada system to actively add a NoExecute taint to the target cluster. This decision needs to be actively made by users.

We can learn from the experience of `kubectl` and provide users with a separate `karmadactl drain` command to evict resources on the target cluster. In addition, Karmada provides a strategy for [resource retention on clusters](https://karmada.io/docs/next/tutorials/resource-migration/#step-5-set-preserveresourcesondeletion-for-propagationpolicy), and users can define whether to retain resources in member clusters through this strategy.
...
2. When the cluster health status is determined to be abnormal, the system will not automatically add the NoExecute taint to the target cluster. Users need to define the TaintClusterByCondition policy.
3. The Karmada system will no longer add a default 300 second tolerance for the NoExecute taints with keys `cluster.karmada.io/not-ready` or `cluster.karmada.io/unschedulable`.
