...
If preemption happens in such scenarios, please fell free to file an issue.
...
This proposal are going to add a new API object called `PriorityClass`. The priority class defines the mapping between the priority name and its value.
...
type PriorityClass struct {
    ...
    // Value represents the integer value of this priority class. This is the actual priority that bindings
    // receive when they match the name of this class.
    ...
    // PreemptionPolicy iS the Policy for preempting bindings with lower priority.
...
type PreemptionPolicy struct {
    ...
    // GracePreemptionPeriodSeconds is optional duration in seconds the binding needs to terminate gracefully.
    // Value must be non-negative integer. The value zero indicates preempt immediately.
    // If this value is nil, the default grace period will be used instead.
    // The grace period is the duration in seconds after the processes running in the binding are cleaned
    // up the previous scheduling result.
    // Set this value longer than the expected cleanup time for your process.
    // Defaults to 0 seconds.
    // +optional
    GracePreemptionPeriodSeconds *int64 `json:"gracePreemptionPeriodSeconds,omitempty"
}
```

```go
// PreemptionBehavior describes whether and how to preempt a binding.
// +enum
...
type Placement struct {
    ...
    // If not specified, the binding priority will be zero, binding preemptionPolicy will be Never.
    // +optional
...
**Important notes**: The scheduler will only consider bindings whose resource requirements are not empty.
...
4. That is `resourceBindingSpec.gracePreemptionPeriodSeconds`, if it's more than 0, we'll retry to schedule the pending binding after `resourceBindingSpec.gracePreemptionPeriodSeconds`.

### Feature gate

This binding preemption feature is an experimental feature, so we should add a feature gate to control the global enablement.
