...
type Placement struct {
  // WorkloadAffinity represents the inter-workload affinity or anti-affinity scheduling policies.
  // +optional
...
type WorkloadAffinity struct {
    ...
    //
    // For the first workload of an affinity group (when no workloads with matching
    // label value exist in the system), the scheduler will not block scheduling.
    // This allows bootstrapping new workload groups without encountering scheduling
    // deadlocks, providing the best user experience.
    //
    // +optional
...
type WorkloadAntiAffinityTerm struct {
    ...
    // belong to the same affinity group and should be separated.
    //
    ...
    // will avoid clusters where other "app.group=frontend" workloads exist.
    //
    // +required
...
type ResourceBindingSpec struct {
	...
	// separated across clusters. Populated by controllers, the scheduler consumes it for decisions.
	// +optional
