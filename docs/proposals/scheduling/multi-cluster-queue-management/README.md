...
Namespaces without a `TenantQueue` fall through to global defaults (`BestEffortFIFO`).
...
Routing: the namespace is extracted from the `NamespacedKey` of each `QueuedBindingInfo`. `ClusterResourceBinding` objects (no namespace) always go to the default queue.
...
Gated behind `SchedulerQueueManagement` (alpha, disabled by default). Requires `PriorityBasedScheduling` to also be enabled.
...
6. Feature gate: `SchedulerQueueManagement` (disabled by default).
...
3. Graduation of `SchedulerQueueManagement` feature gate to beta.
