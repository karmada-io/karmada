...
func ConvertLuaResultInto(luaResult *lua.LTable, obj interface{}, references []*unstructured.Unstructured) error {
...
func convertEmptySliceToEmptyStructPartly(objBytes []byte, isStruct bool, references []*unstructured.Unstructured) ([]byte, error) {
	// 1. if an empty lua object is originally struct type (not slice), just return {}
	if bytes.Equal(objBytes, []byte("[]")) && isStruct {
		return []byte("{}"), nil
	}
