...
func (o *objectWatcherImpl) allowUpdate(clusterName string, desiredObj, clusterObj *unstructured.Unstructured) bool {
	...
	if util.GetLabelValue(desiredObj.GetLabels(), workv1alpha2.WorkPermanentIDLabel) == util.GetLabelValue(clusterObj.GetLabels(), workv1alpha2.WorkPermanentIDLabel) {
		return true
	}
	klog.Warningf("The existing resource(kind=%s, %s/%s) in the cluster(%s) is not managed by Karmada, cause: (desired: %s=%s, observed: %s=%s).",
		clusterObj.GetKind(), clusterObj.GetNamespace(), clusterObj.GetName(), clusterName,
		workv1alpha2.WorkPermanentIDLabel, util.GetLabelValue(desiredObj.GetLabels(), workv1alpha2.WorkPermanentIDLabel),
