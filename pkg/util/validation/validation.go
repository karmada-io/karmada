...
func validateResourceSelectors(selectors []policyv1alpha1.ResourceSelector, policyNamespace string, fldPath *field.Path) field.ErrorList {
	...
	for index, selector := range selectors {
		if len(selector.Namespace) == 0 {
			allErrs = append(allErrs, field.Invalid(fldPath.Index(index).Child("namespace"), selector.Namespace, "namespace is required, it should be set by the mutating admission webhook"))
		} else if selector.Namespace != policyNamespace {
			allErrs = append(allErrs, field.Invalid(fldPath.Index(index).Child("namespace"), selector.Namespace, "namespace of resource selector must be equal to the namespace of the policy"))
		}
	}
	return allErrs
}
