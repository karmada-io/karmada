...
func newGoldenRawObject(name string) *dynamic.RawObject {
	obj, err := dynamic.NewRawObject(fmt.Appendf(nil, `{"metadata":{"name":%q}}`, name))
	if err != nil {
		panic(err)
	}
	return obj
...
func rawUnstructuredContent(t *testing.T, obj *dynamic.RawObject) map[string]any {
	t.Helper()
...
func rawListUnstructuredContent(t *testing.T, list *dynamic.RawObjectList) map[string]any {
	t.Helper()
	items := make([]any, 0, len(list.Items))
	for i := range list.Items {
		u, err := list.Items[i].ToUnstructured()
		if err != nil {
			t.Fatal(err)
		}
		u.SetAPIVersion(list.Items[i].APIVersion)
		u.SetKind(list.Items[i].Kind)
...
func normalizeHTTPFixture(data []byte) []byte {
...
func TestGoldenResponse(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			response: "nonlist",
			do: func(t *testing.T, client dynamic.ResourceInterface) any {
				got, err := client.Create(context.Background(), &dynamic.RawObject{}, metav1.CreateOptions{})
				if err != nil {
					t.Fatal(err)
				}

				return rawUnstructuredContent(t, got)
			},
		},
		{
			name:     "update",
			response: "nonlist",
			do: func(t *testing.T, client dynamic.ResourceInterface) any {
				got, err := client.Update(context.Background(), newGoldenRawObject("name"), metav1.UpdateOptions{})
				if err != nil {
					t.Fatal(err)
				}
		...
		},
		{
			...
			response: "nonlist",
			do: func(t *testing.T, client dynamic.ResourceInterface) any {
				got, err := client.UpdateStatus(context.Background(), newGoldenRawObject("name"), metav1.UpdateOptions{})
		...
		},
		{
			...
			do: func(t *testing.T, client dynamic.ResourceInterface) any {
				...
				return rawListUnstructuredContent(t, got)
			},
		},
		{
			name:     "watch",
