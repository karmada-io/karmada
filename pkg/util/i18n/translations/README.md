...
Edit the appropriate `karmada.po` file, `poedit` is a popular open source tool
for translations. You can load the `pkg/util/i18n/translations/kubectl/template.pot` file
to find messages that might be missing.

Once you are done with your `karmada.po` file, generate the corresponding `karmada.mo`
...
There is a script in `pkg/util/i18n/translations/extract.py` that knows how to do some
