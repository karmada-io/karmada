...
func TestMutatingAdmission_Handle_FullCoverage(t *testing.T) {
	// Define the rb name and namespace to be used in the test.
	name := "test-resource-binding"
	...
	// Create the initial rb object with default values for testing.
	...
	// Define the expected rb object after mutations.
	wantRB := &workv1alpha2.ResourceBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
			Labels: map[string]string{
				workv1alpha2.ResourceBindingPermanentIDLabel: "some-unique-uuid",
			},
		},
		Spec: workv1alpha2.ResourceBindingSpec{
			Resource: workv1alpha2.ObjectReference{
	...
	// Mock decoder that decodes the request into the rb object.
	decoder := &fakeMutationDecoder{
		obj: rb,
	}

	// Marshal the expected rb object to simulate the final mutated object.
	wantBytes, err := json.Marshal(wantRB)
	if err != nil {
		t.Fatalf("Failed to marshal expected resource binding: %v", err)
	}
	...
	// If any other patches are present, it indicates that the rb object was not handled as expected.
	firstPatch := got.Patches[0]
	if firstPatch.Operation != "replace" || firstPatch.Path != "/metadata/labels/resourcebinding.karmada.io~1permanent-id" {
