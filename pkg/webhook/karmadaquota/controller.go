...
type quotaEvaluator struct {
	...
	lockAcquisitionFunc func([]v1alpha1.KarmadaQuota) func()
...
func NewQuotaEvaluator(quotaAccessor QuotaAccessor, ignoredResources map[schema.GroupResource]struct{}, quotaRegistry quota.Registry, lockAcquisitionFunc func([]v1alpha1.KarmadaQuota) func(), config *resourcequotaapi.Configuration, workers int, stopCh <-chan struct{}) Evaluator {
...
func (e *quotaEvaluator) doWork() {
	workFunc := func() bool {
		...
		klog.Infof("doWork namespace:%s, admissionAttributes len:%d, name0:%s",
			ns, len(admissionAttributes), admissionAttributes[0].attributes.GetName())
...
func (e *quotaEvaluator) checkQuotas(quotas []v1alpha1.KarmadaQuota, admissionAttributes []*admissionWaiter, remainingRetries int) {
	...
	klog.Infof("checkQuotas: start check quotas.")
	...
	var updatedFailedQuotas []v1alpha1.KarmadaQuota
	var lastErr error
	for i := range quotas {
		newQuota := quotas[i]
	...
	quotasToCheck := []v1alpha1.KarmadaQuota{}
...
func copyQuotas(in []v1alpha1.KarmadaQuota) ([]v1alpha1.KarmadaQuota, error) {
	out := make([]v1alpha1.KarmadaQuota, 0, len(in))
...
func getMatchedLimitedScopes(evaluator quota.Evaluator, inputObject runtime.Object, limitedResources []resourcequotaapi.LimitedResource) ([]corev1.ScopedResourceSelectorRequirement, error) {
	...
	for _, limitedResource := range limitedResources {
		...
		for _, item := range limitedResource.MatchScopes {
			MatchScopes = append(MatchScopes, corev1.ScopedResourceSelectorRequirement{
				ScopeName: corev1.ResourceQuotaScope(item.ScopeName),
				Operator:  corev1.ScopeSelectorOperator(item.Operator),
				Values:    item.Values,
			})
		}
		matched, err := evaluator.MatchingScopes(inputObject, MatchScopes)
		if err != nil {
...
func (e *quotaEvaluator) checkRequest(quotas []v1alpha1.KarmadaQuota, a admission.Attributes) ([]v1alpha1.KarmadaQuota, error) {
...
func CheckRequest(quotas []v1alpha1.KarmadaQuota, a admission.Attributes, evaluator quota.Evaluator,
	...
	for _, index := range interestingQuotaIndexes {
		...
		klog.Infof("match quota[%s][%s] success. requested[%s], newuseage[%s]", resourceQuota.Namespace, resourceQuota.Name,
			prettyPrint(deltaUsage), prettyPrint(newUsage))
...
func getScopeSelectorsFromQuota(quota v1alpha1.KarmadaQuota) []corev1.ScopedResourceSelectorRequirement {
...
func (e *quotaEvaluator) addWork(a *admissionWaiter) {
	...
	klog.V(4).Infof("addWork namespace:%q, name:%q", ns, a.attributes.GetName())
...
func hasUsageStats(resourceQuota *v1alpha1.KarmadaQuota, interestingResources []corev1.ResourceName) bool {
