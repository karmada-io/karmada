...
func (a *MutatingAdmission) Handle(_ context.Context, req admission.Request) admission.Response {
	...
	permanentID := work.Labels[workv1alpha2.WorkPermanentIDLabel]
	klog.V(2).Infof("Mutating the work(permanentID=%s, %s/%s) for request: %s", permanentID, work.Namespace, work.Name, req.Operation)
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		util.MergeLabel(workloadObj, workv1alpha2.WorkPermanentIDLabel, permanentID)
