...
func (d *DependenciesDistributor) removeScheduleResultFromAttachedBindings(bindingNamespace, bindingName string, attachedBindings []*workv1alpha2.ResourceBinding) error {
	...
	for index, binding := range attachedBindings {
		...
		if len(updatedSnapshot) == 0 {
			// No more parents: clear preserve to default false and leave CR as-is (backward compatible behavior).
			attachedBindings[index].Spec.PreserveResourcesOnDeletion = nil
		} else {
			// Re-aggregate from remaining parents.
			aggCR, aggPreserve, hasConflict, details, aggErr := d.aggregatePolicyForAttachedBinding(context.TODO(), updatedSnapshot)
			if aggErr != nil {
				klog.Errorf("Failed to aggregate after parent removal for binding(%s/%s): %v", binding.Namespace, binding.Name, aggErr)
			...
			if binding.Spec.ConflictResolution != aggCR {
				attachedBindings[index].Spec.ConflictResolution = aggCR
				needUpdate = true
			}
			if ptr.Deref(binding.Spec.PreserveResourcesOnDeletion, false) != ptr.Deref(aggPreserve, false) {
				attachedBindings[index].Spec.PreserveResourcesOnDeletion = aggPreserve
				needUpdate = true
			}
			if hasConflict {
...
func (d *DependenciesDistributor) createOrUpdateAttachedBinding(attachedBinding *workv1alpha2.ResourceBinding) error {
	...
	if err == nil {
		...
		// Always merge relationship and labels first, so that aggregation sees the latest parents.
		...
		if existBinding.Spec.Placement == nil {
			...
			if existBinding.Spec.ConflictResolution != aggCR {
				existBinding.Spec.ConflictResolution = aggCR
				needUpdate = true
			}
			if ptr.Deref(existBinding.Spec.PreserveResourcesOnDeletion, false) != ptr.Deref(aggPreserve, false) {
				existBinding.Spec.PreserveResourcesOnDeletion = aggPreserve
				needUpdate = true
			}
			if hasConflict {
		...
		// For explicit RBs, only persist relationship/labels/resource if changed.
		if err := d.Client.Update(context.TODO(), existBinding); err != nil {
			klog.Errorf("Failed to update resourceBinding(%s): %v", bindingKey, err)
			return err
		}
...
// aggregatePolicyForAttachedBinding aggregates policy fields from all parents in RequiredBy.
// Returns: (conflictResolution, preserveOnDeletion, hasConflict, details)
func (d *DependenciesDistributor) aggregatePolicyForAttachedBinding(ctx context.Context, requiredBy []workv1alpha2.BindingSnapshot) (policyv1alpha1.ConflictResolution, *bool, bool, string, error) {
	if len(requiredBy) == 0 {
		// No parents, leave values unset to let defaults apply downstream.
		return "", nil, false, "", nil
	}

	anyOverwrite := false
	anyPreserve := false
	// Track value diversity to report conflicts.
	confSet := make(map[policyv1alpha1.ConflictResolution]struct{})
	preserveSet := make(map[bool]struct{})
	var details string

	for _, snap := range requiredBy {
		// Fetch parent binding by snapshot (RB if Namespace set; otherwise CRB).
		var cr policyv1alpha1.ConflictResolution
		var pr *bool

		if len(snap.Namespace) == 0 {
			// ClusterResourceBinding
			parent := &workv1alpha2.ClusterResourceBinding{}
			if err := d.Client.Get(ctx, client.ObjectKey{Name: snap.Name}, parent); err != nil {
				// If parent not found, skip it.
				if apierrors.IsNotFound(err) {
					continue
				}
				return "", nil, false, "", err
			}
			cr = parent.Spec.ConflictResolution
			pr = parent.Spec.PreserveResourcesOnDeletion
			details += fmt.Sprintf("CRB/%s(cr=%s,preserve=%v); ", parent.Name, cr, ptr.Deref(pr, false))
		} else {
			// ResourceBinding
			parent := &workv1alpha2.ResourceBinding{}
			if err := d.Client.Get(ctx, client.ObjectKey{Namespace: snap.Namespace, Name: snap.Name}, parent); err != nil {
				if apierrors.IsNotFound(err) {
					continue
				}
				return "", nil, false, "", err
			...
			cr = parent.Spec.ConflictResolution
			pr = parent.Spec.PreserveResourcesOnDeletion
			details += fmt.Sprintf("RB/%s/%s(cr=%s,preserve=%v); ", parent.Namespace, parent.Name, cr, ptr.Deref(pr, false))
		}
		...
		if ptr.Deref(pr, false) {
			anyPreserve = true
		}
		preserveSet[ptr.Deref(pr, false)] = struct{}{}
	}

	// Apply aggregation rules: non-default values take precedence.
	aggCR := policyv1alpha1.ConflictAbort
	if anyOverwrite {
		aggCR = policyv1alpha1.ConflictOverwrite
	}
	var aggPreserve *bool
	if anyPreserve {
		aggPreserve = ptr.To(true)
	} else {
		aggPreserve = ptr.To(false)
	}

	hasConflict := (len(confSet) > 1) || (len(preserveSet) > 1)
	return aggCR, aggPreserve, hasConflict, details, nil
}
