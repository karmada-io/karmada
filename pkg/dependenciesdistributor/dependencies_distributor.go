...
func (d *DependenciesDistributor) createOrUpdateAttachedBinding(ctx context.Context, attachedBinding *workv1alpha2.ResourceBinding) error {
	...
	if err == nil {
		...
		if existBinding.Spec.Placement == nil {
			crConflictDetected, err := d.conflictDetectedForConflictResolution(ctx, mergedRequiredBy)
			if err != nil {
				klog.Errorf("Failed to check conflict for resourceBinding(%s): %v", bindingKey, err)
				return err
			}

			preserveConflictDetected, err := d.conflictDetectedForPreserveOnDeletion(ctx, mergedRequiredBy)
			if err != nil {
