	...
	for index, binding := range attachedBindings {
		...
		if attachedBindings[index].Spec.Placement == nil {
			...
			if len(updatedSnapshot) == 0 {
				...
				// Recompute aggregated strategy from remaining RequiredBy.
				aggCR, _, err := d.AggregateAndDetectConflictResolution(context.TODO(), updatedSnapshot)
				if err != nil {
					klog.Errorf("Failed to aggregate ConflictResolution for resourceBinding(%s/%s): %v", binding.Namespace, binding.Name, err)
					errs = append(errs, err)
				} else {
					attachedBindings[index].Spec.ConflictResolution = aggCR
				}

				aggPreserve, _, err := d.AggregateAndDetectPreserveOnDeletion(context.TODO(), updatedSnapshot)
				if err != nil {
					klog.Errorf("Failed to aggregate PreserveResourcesOnDeletion for resourceBinding(%s/%s): %v", binding.Namespace, binding.Name, err)
					errs = append(errs, err)
				} else {
					attachedBindings[index].Spec.PreserveResourcesOnDeletion = ptr.To(aggPreserve)
				}
			}
		}
...
func (d *DependenciesDistributor) createOrUpdateAttachedBinding(attachedBinding *workv1alpha2.ResourceBinding) error {
	...
	if err == nil {
		...
		if existBinding.Spec.Placement == nil {
			// Aggregate policies from referencing ResourceBindings and detect conflicts.
			aggCR, crConflicted, err := d.AggregateAndDetectConflictResolution(context.TODO(), mergedRequiredBy)
			if err != nil {
				klog.Errorf("Failed to aggregate ConflictResolution for resourceBinding(%s): %v", bindingKey, err)
				return err
			}


			aggPreserve, preserveConflicted, err := d.AggregateAndDetectPreserveOnDeletion(context.TODO(), mergedRequiredBy)
			if err != nil {
				klog.Errorf("Failed to aggregate PreserveResourcesOnDeletion for resourceBinding(%s): %v", bindingKey, err)
				return err
			}
			...
			// Update with aggregation results.
			existBinding.Spec.ConflictResolution = aggCR
			existBinding.Spec.PreserveResourcesOnDeletion = ptr.To(aggPreserve)
		}
