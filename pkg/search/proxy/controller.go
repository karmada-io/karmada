...
func (ctl *Controller) reconcile(util.QueueKey) error {
	...
	allMatchedResources := make(map[schema.GroupVersionResource]struct{})
	...
	for _, registry := range registries {
		...
		for _, selector := range registry.Spec.ResourceSelectors {
			...
			allMatchedResources[gvr] = struct{}{}
		}
	...
	return ctl.store.UpdateCache(resourcesByClusters, allMatchedResources)
}
