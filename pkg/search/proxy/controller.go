...
func (ctl *Controller) reconcile(util.QueueKey) error {
	...
	resourcesByClusters := make(map[string]map[schema.GroupVersionResource]*store.NamespaceScope)
	for _, registry := range registries {
		matchedResources := make(map[schema.GroupVersionResource][]string, len(registry.Spec.ResourceSelectors))
		for _, selector := range registry.Spec.ResourceSelectors {
			...
			ns := matchedResources[gvr]
			ns = append(ns, selector.Namespace)
			matchedResources[gvr] = ns
		}
		...
		for _, cluster := range clusters {
			...
			if _, exist := resourcesByClusters[cluster.Name]; !exist {
				resourcesByClusters[cluster.Name] = make(map[schema.GroupVersionResource]*store.NamespaceScope)
			}

			for resource, ns := range matchedResources {
				if _, exist := resourcesByClusters[cluster.Name][resource]; !exist {
					resourcesByClusters[cluster.Name][resource] = &store.NamespaceScope{}
				}
				resourcesByClusters[cluster.Name][resource].AddAll(ns)
			}
		}
	}
