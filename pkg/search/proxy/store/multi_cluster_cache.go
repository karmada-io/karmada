...
type Store interface {
	UpdateCache(resourcesByCluster map[string]map[schema.GroupVersionResource]*MultiNamespace, allMatchedResources map[schema.GroupVersionResource]struct{}) error
...
type MultiClusterCache struct {
	...
	matchedResources map[schema.GroupVersionResource]struct{}
...
func (c *MultiClusterCache) UpdateCache(resourcesByCluster map[string]map[schema.GroupVersionResource]*MultiNamespace, allMatchedResources map[schema.GroupVersionResource]struct{}) error {
	if klog.V(3).Enabled() {
		start := time.Now()
		defer func() {
	...
	c.matchedResources = allMatchedResources
	return nil
}
...
// HasResource return whether resource is matched.
func (c *MultiClusterCache) HasResource(resource schema.GroupVersionResource) bool {
	c.lock.RLock()
	defer c.lock.RUnlock()
	_, ok := c.matchedResources[resource]
	return ok
}
