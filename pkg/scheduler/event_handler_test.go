...
func TestResourceBindingEventFilter(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			obj: createResourceBinding("test-rb", "test-scheduler", map[string]string{
				workv1alpha2.BindingManagedByLabel: "test-manager",
			}, &policyv1alpha1.Suspension{Scheduling: ptr.To(true)}),
			expectedResult: false,
		},
		{
...
func createResourceBinding(name, schedulerName string, labels map[string]string, suspension *policyv1alpha1.Suspension) *workv1alpha2.ResourceBinding {
...
func createClusterResourceBinding(name, schedulerName string, labels map[string]string, suspension *policyv1alpha1.Suspension) *workv1alpha2.ClusterResourceBinding {
