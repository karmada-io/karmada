...
func TestDoScheduleBinding(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectError, err != nil, "Unexpected error status")
			// Inferring scheduling occurred based on the presence of patch actions.
			actions := fakeClient.Actions()
			patchActions := filterPatchActions(actions)
			if tt.expectSchedule {
				assert.NotEmpty(t, patchActions, "Expected patch actions, but got none")
			} else {
				assert.Empty(t, patchActions, "Unexpected patch actions")
			}
		})
	}
}
...
func TestDoScheduleClusterBinding(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectError:    false,
			expectedPatches: []string{
				`{"metadata":{"annotations":{"policy.karmada.io/applied-placement":"{\"clusterAffinity\":{\"clusterNames\":[\"cluster1\",\"cluster2\"]}}"}},"spec":{"clusters":[{"name":"cluster1","replicas":1},{"name":"cluster2","replicas":1}]}}`,
				`{"status":{"conditions":[{"message":"Binding has been scheduled successfully.","reason":"Success","status":"True","type":"Scheduled"}]}}`,
			},
		},
		{
			name: "deleting cluster binding",
			binding: &workv1alpha2.ClusterResourceBinding{
				ObjectMeta: metav1.ObjectMeta{
		...
		},
		{
			...
			binding: &workv1alpha2.ClusterResourceBinding{
				ObjectMeta: metav1.ObjectMeta{
					Name: "replicas-changed-binding",
					Annotations: map[string]string{
						util.PolicyPlacementAnnotation: `{"replicaScheduling":{"replicaDivisionPreference":"Weighted","replicaSchedulingType":"Divided"}}`,
					},
				},
				Spec: workv1alpha2.ResourceBindingSpec{
					Replicas: 2,
			...
			expectError:    false,
			expectedPatches: []string{
				`{"metadata":{"annotations":{"policy.karmada.io/applied-placement":"{\"replicaScheduling\":{\"replicaSchedulingType\":\"Divided\",\"replicaDivisionPreference\":\"Weighted\"}}"}},"spec":{"clusters":[{"name":"cluster1","replicas":1},{"name":"cluster2","replicas":1}]}}`,
				`{"status":{"conditions":[{"message":"Binding has been scheduled successfully.","reason":"Success","status":"True","type":"Scheduled"}],"schedulerObservedGeneration":null}}`,
			},
		},
	}
			...
			if tt.expectSchedule {
				...
				for i, expectedPatch := range tt.expectedPatches {
					if i < len(patchActions) {
						...
						assert.True(t, compareJSONIgnoreTimestamps(t, expectedPatch, actualPatch), "Patch %d does not match expected", i+1)
					}
				}
			} else {
				assert.Empty(t, patchActions, "Unexpected patch actions")
...
func TestRecordScheduleResultEventForResourceBinding(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			schedulerErr:   nil,
			expectedEvents: []string{
				"Normal ScheduleBindingSucceed Binding has been scheduled successfully. Result: {cluster1:1}",
				"Normal ScheduleBindingSucceed Binding has been scheduled successfully. Result: {cluster1:1}",
			},
		},
		{
			...
			scheduleResult: nil,
			schedulerErr:   errors.New("scheduling failed"),
			expectedEvents: []string{
				"Warning ScheduleBindingFailed scheduling failed",
				"Warning ScheduleBindingFailed scheduling failed",
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fakeRecorder := record.NewFakeRecorder(10)
			s := &Scheduler{eventRecorder: fakeRecorder}

			s.recordScheduleResultEventForResourceBinding(tt.rb, tt.scheduleResult, tt.schedulerErr)

			var receivedEvents []string
			timeout := time.After(time.Second)
			expectedEventCount := len(tt.expectedEvents)
			...
			assert.Equal(t, expectedEventCount, len(receivedEvents), "Unexpected number of events recorded")
			for i, expectedEvent := range tt.expectedEvents {
				if i < len(receivedEvents) {
					assert.Equal(t, expectedEvent, receivedEvents[i], "Event content mismatch")
				}
			}
		})
	}
...
func TestPlacementChanged(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test for ClusterResourceBinding
			...
			got := placementChanged(*crb.Spec.Placement, tt.appliedPlacementStr, crb.Status.SchedulerObservedAffinityName)
			assert.Equal(t, tt.want, got, "placementChanged() for ClusterResourceBinding result mismatch")

			// Test for ResourceBinding
			rb := &workv1alpha2.ResourceBinding{
				ObjectMeta: metav1.ObjectMeta{
			...
			assert.Equal(t, tt.want, got, "placementChanged() for ResourceBinding result mismatch")
		})
	}
}
...
// Helper function to compare JSON strings while ignoring timestamp fields
func compareJSONIgnoreTimestamps(t *testing.T, expected, actual string) bool {
	...
	// Remove timestamp fields from both maps
	removeTimestamps(expectedMap)
	removeTimestamps(actualMap)

	return assert.Equal(t, expectedMap, actualMap)
}

// Helper function to recursively remove timestamp fields from a map
func removeTimestamps(m map[string]interface{}) {
	for k, v := range m {
		if k == "lastTransitionTime" || k == "lastScheduledTime" {
			delete(m, k)
		} else if subMap, ok := v.(map[string]interface{}); ok {
			removeTimestamps(subMap)
