...
func TestExtractExistedClusters(t *testing.T) {
	tests := []struct {
		name             string
		spec             policyv1alpha1.FederatedResourceQuotaSpec
		...
		{
			name: "the clusters declared in StaticAssignments are all existed",
			spec: policyv1alpha1.FederatedResourceQuotaSpec{
				StaticAssignments: []policyv1alpha1.StaticClusterAssignment{
					{
						ClusterName: "cluster1",
			...
			clusterList: &clusterv1alpha1.ClusterList{Items: []clusterv1alpha1.Cluster{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "cluster1",
					},
		...
		{
			name: "not all clusters declared in StaticAssignments exist",
			spec: policyv1alpha1.FederatedResourceQuotaSpec{
				StaticAssignments: []policyv1alpha1.StaticClusterAssignment{
					{
						ClusterName: "cluster1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractExistedClusters(tt.spec, tt.clusterList)
			assert.ElementsMatch(t, tt.expectedClusters, result)
		})
	}
}
