...
func (c *WorkStatusController) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	klog.V(4).InfoS("Reconciling status of Work.", "work", req.NamespacedName.String())
	...
	if !util.IsClusterReady(&cluster.Status) {
		...
		klog.ErrorS(err, "Stop syncing the Work to the cluster as not ready.", "work", work.Namespace+"/"+work.Name, "cluster", cluster.Name)
		return controllerruntime.Result{}, err
	}
...
func (c *WorkStatusController) buildResourceInformers(cluster *clusterv1alpha1.Cluster, work *workv1alpha1.Work) (controllerruntime.Result, error) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to register informer for Work.", "work", work.GetNamespace()+"/"+work.GetName())
		return controllerruntime.Result{}, err
	}
	return controllerruntime.Result{}, nil
}
...
func getClusterNameFromAnnotation(resource *unstructured.Unstructured) (string, error) {
	...
	if !exist {
		klog.V(5).InfoS("Ignore resource which is not managed by Karmada.", "kind", resource.GetKind(), "work", resource.GetNamespace()+"/"+resource.GetName())
		return "", nil
	}
...
func (c *WorkStatusController) syncWorkStatus(key util.QueueKey) error {
	...
	if !ok {
		klog.Error("Failed to sync status, invalid key", "key", key)
	...
	if err := c.Client.Get(ctx, client.ObjectKey{Namespace: workNamespace, Name: workName}, workObject); err != nil {
		...
		klog.ErrorS(err, "Failed to get Work from cache", "work", workNamespace+"/"+workName)
		return err
	}
	...
	klog.InfoS("Reflecting resource status to Work.", "kind", observedObj.GetKind(), "resource", observedObj.GetNamespace()+"/"+observedObj.GetName(), "work", workNamespace+"/"+workName)
...
func (c *WorkStatusController) updateAppliedCondition(ctx context.Context, work *workv1alpha1.Work, status metav1.ConditionStatus, reason, message string) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to update condition of work.", "work", work.Namespace+"/"+work.Name)
	}
}
...
func (c *WorkStatusController) getGVRsFromWork(work *workv1alpha1.Work) (map[schema.GroupVersionResource]bool, error) {
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to get GVR from GVK for resource.", "resource", workload.GetNamespace()+"/"+workload.GetName())
			return nil, err
		}
		gvrTargets[gvr] = true
	}
...
func (c *WorkStatusController) eventf(object *unstructured.Unstructured, eventType, reason, messageFmt string, args ...interface{}) {
	...
	if err != nil {
		klog.ErrorS(err, "Ignoring event. Faild to build event reference for: kind=%s, %s", "reason", reason, "kind", object.GetKind(), "reference", klog.KObj(object))
		return
	}
