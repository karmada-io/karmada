...
func ensureWork(
	...
	for i := range targetClusters {
		...
		// If and only if the resource template has replicas, we need to revise replicas.
		// The replicas value in "Work" should be consistent with the scheduling result bindingSpec.Clusters,
		// or bindingSpec.RequiredBy controlled by the dependencies-distributor, rather than bindingSpec.Replicas.
		// This remains true even when the replica scheduling policy is duplicated.
		// In certain scenarios, such as scheduling suspension or insufficient federation quota,
		// bindingSpec.Replicas should not be directly synchronized to the member clusters.
		...
		if needReviseJobCompletions(bindingSpec.Replicas, bindingSpec.Placement) && len(jobCompletions) > 0 {
