...
func mergeLabel(workload *unstructured.Unstructured, workNamespace string, binding metav1.Object, scope apiextensionsv1.ResourceScope) map[string]string {
	...
	if scope == apiextensionsv1.NamespaceScoped {
		...
		util.MergeLabel(workload, workv1alpha2.ResourceBindingReferenceUID, string(binding.GetUID()))
		...
		workLabel[workv1alpha2.ResourceBindingReferenceUID] = string(binding.GetUID())
	} else {
		...
		util.MergeLabel(workload, workv1alpha2.ClusterResourceBindingReferenceUID, string(binding.GetUID()))
		workLabel[workv1alpha2.ClusterResourceBindingReferenceKey] = names.GenerateBindingReferenceKey("", binding.GetName())
		workLabel[workv1alpha2.ClusterResourceBindingReferenceUID] = string(binding.GetUID())
	}
	return workLabel
}
