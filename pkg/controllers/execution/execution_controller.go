...
func (c *Controller) tryDeleteWorkload(clusterName string, work *workv1alpha1.Work) error {
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		orphaningDeletionPolicy := workload.GetAnnotations()[workv1alpha1.OrphaningDeletionPolicyAnnotation]
		switch orphaningDeletionPolicy {
		case workv1alpha1.OrphaningDeletionCompleteAnnotation:
			klog.V(4).Infof("Found orphaningDeletion (%s) on %s. cluster is %s, Removing label/annotation.", orphaningDeletionPolicy, work.Name, clusterName)
