...
func (c *Controller) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	logger.Info("Reconciling Work %s", req.NamespacedName.String())
	...
	if err != nil {
		logger.Error(err, "Failed to get the given member cluster", "cluster", clusterName)
		return controllerruntime.Result{}, err
	}
	...
	if err := c.updateWorkDispatchingConditionIfNeeded(ctx, work); err != nil {
		logger.Error(err, "Failed to update work condition type", "type", workv1alpha1.WorkDispatching)
		return controllerruntime.Result{}, err
	}
...
func (c *Controller) cleanupPolicyClaimMetadata(ctx context.Context, work *workv1alpha1.Work, cluster *clusterv1alpha1.Cluster) error {
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		if err != nil {
			logger.Error(err, "Failed to update metadata in the given member cluster", "cluster", cluster.Name)
			return err
		}
	}
...
func (c *Controller) tryDeleteWorkload(ctx context.Context, clusterName string, work *workv1alpha1.Work) error {
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		if err != nil {
			logger.Error(err, "Failed to unmarshal workload")
			return err
		}
		...
		if err != nil {
			logger.Error(err, "Failed to delete resource in the given member cluster", "cluster", clusterName)
			return err
		}
	}
...
func (c *Controller) syncToClusters(ctx context.Context, clusterName string, work *workv1alpha1.Work) error {
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		c.eventf(ctx, workload, corev1.EventTypeNormal, events.EventReasonSyncWorkloadSucceed, "Successfully applied resource(%v/%v) to cluster %s", workload.GetNamespace(), workload.GetName(), clusterName)
		syncSucceedNum++
	}
...
func (c *Controller) tryCreateOrUpdateWorkload(ctx context.Context, clusterName string, workload *unstructured.Unstructured) error {
	...
	if err != nil {
		logger.Error(err, "Failed to get FederatedKey", "name", workload.GetName())
		return err
	}
...
func (c *Controller) updateWorkDispatchingConditionIfNeeded(ctx context.Context, work *workv1alpha1.Work) error {
	...
	c.eventf(ctx, obj, corev1.EventTypeNormal, events.EventReasonWorkDispatching, newWorkDispatchingCondition.Message)
	return nil
}
...
func (c *Controller) eventf(ctx context.Context, object *unstructured.Unstructured, eventType, reason, messageFmt string, args ...interface{}) {
	logger := log.FromContext(ctx)
	ref, err := util.GenEventRef(object)
	if err != nil {
		logger.Error(err, "Ignore event as failed to build event reference", "reason", reason, "kind", object.GetKind(), "object", object.GetName())
		return
	}
