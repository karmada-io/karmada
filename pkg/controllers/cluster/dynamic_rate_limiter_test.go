...
func TestGracefulEvictionRateLimiter_ExponentialBackoff(t *testing.T) {
	...
	for i := 1; i < len(attemptTimes); i++ {
		...
		// In CI environments, CPU contention can cause inconsistent delays (e.g., attempt N-1 overslept significantly,
		// making it larger than attempt N even if the expected delay for N is larger).
		// We disable the strict monotonic check "observedDelay < lastObservedDelay" to avoid flakes.
		// The lower-bound check below is sufficient to verify backoff behavior.
		/*
			if i > 1 {
