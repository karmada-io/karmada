...
func TestEvictionWorker_ThroughputWithDynamicRateLimiter(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "healthy state with high rate",
			clusterList: []runtime.Object{makeCluster("c1", true), makeCluster("c2", true)},
			evictionOpts: EvictionQueueOptions{
		...
		{
			name:        "unhealthy large-scale with secondary rate",
			clusterList: newClusterList(15, 8),
			evictionOpts: EvictionQueueOptions{
				ResourceEvictionRate:          50,
				SecondaryResourceEvictionRate: 2,
				UnhealthyClusterThreshold:     0.5,
		...
		{
			name:        "unhealthy small-scale should halt",
			clusterList: []runtime.Object{makeCluster("c1", true), makeCluster("c2", false), makeCluster("c3", false)}, // 3 total, 2 unhealthy
			evictionOpts: EvictionQueueOptions{
				ResourceEvictionRate:      50,
				UnhealthyClusterThreshold: 0.5,
				LargeClusterNumThreshold:  10,
		...
		{
			name:      "lister error should halt",
			listError: errors.New("informer cache error"),
			evictionOpts: EvictionQueueOptions{
				ResourceEvictionRate: 50,
			},
			expectedMinProcessed: 0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMgr := gmtesting.NewFakeSingleClusterManager(true, true, func(gvr schema.GroupVersionResource) cache.GenericLister {
				if gvr != clusterGVR {
					return nil
				}
				return &fakeGenericLister{objects: tt.clusterList, err: tt.listError}
			})
			...
			opts := EvictionWorkerOptions{
				...
				ControlPlaneInformerManager: mockMgr,
...
// newClusterList is a helper to generate a list of clusters for testing.
func newClusterList(total, unhealthy int) []runtime.Object {
	var out []runtime.Object
	for i := 0; i < total-unhealthy; i++ {
		out = append(out, makeCluster(fmt.Sprintf("healthy-%d", i), true))
	}
	for i := 0; i < unhealthy; i++ {
		out = append(out, makeCluster(fmt.Sprintf("unhealthy-%d", i), false))
	}
	return out
}
