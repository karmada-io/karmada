...
func NewMonitor(informersStarted <-chan struct{}, informerFactory informerfactory.SharedInformerFactory, ignoredResources map[schema.GroupResource]struct{}, resyncPeriod ResyncPeriodFunc, replenishmentFunc ReplenishmentFunc, registry quota.Registry) *QuotaMonitor {
	return &QuotaMonitor{
		informersStarted:  informersStarted,
		informerFactory:   informerFactory,
		ignoredResources:  ignoredResources,
		resourceChanges:   workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "karmada_quota_controller_resource_changes"),
...
func (qm *QuotaMonitor) SyncMonitors(resources map[schema.GroupVersionResource]struct{}) error {
	...
	for resource := range resources {
		...
		if resource.Resource != "resourcebindings" {
			continue
		}
