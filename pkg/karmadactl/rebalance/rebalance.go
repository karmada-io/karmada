...
const (
	rebalancerByCtl        = "karmadactl.karmada.io/rebalancer-by-ctl"
...
func (o *CommandRebalanceOption) getObjInfo(f cmdutil.Factory, args []string) (*unstructured.Unstructured, error) {
	...
	if len(infos) == 0 {
		return nil, fmt.Errorf("the %s/%s does't exist in Karmada", args[0], args[1])
	}
		...
		func(ctx context.Context) (done bool, err error) {
			...
			if err != nil {
				if apierrors.IsNotFound(err) {
					return false, fmt.Errorf("unknown rebalance result, WorkloadRebalancer/%s seems unexpected deleted", rebalancer.Name)
				}
				return false, err
			}
