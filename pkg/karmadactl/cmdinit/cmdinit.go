...
var (
	...
	%[1]s init --crds https://github.com/karmada-io/karmada/releases/download/v1.1.0/crds.tar.gz
	...
	%[1]s init --etcd-image local.registry.com/library/etcd:3.5.1-0
...
func NewCmdInit(cmdOut io.Writer, parentCommand string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:        initShort,
		Long:         initLong,
		Example:      fmt.Sprintf(initExample, parentCommand),
		SilenceUsage: true,
