...
var (
	...
	initExamples = templates.Examples(`
		...
		# Using a configuration file to initialize 
  		%[1]s init --config /path/to/karmada-init.yaml`)
)
...
func NewCmdInit(parentCommand string) *cobra.Command {
	...
	flags.StringVar(&opts.KarmadaInitFilePath, "config", "", "Path to the configuration file for karmadactl init. This file contains all the parameters required for deploying the Karmada control plane. (e.g. --config /path/to/karmada-init.yaml)")
