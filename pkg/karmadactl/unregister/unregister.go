...
func (j *CommandUnregisterOption) AddFlags(flags *pflag.FlagSet) {
	...
	flags.DurationVar(&j.Wait, "wait", 60*time.Second, "wait for the unregister command execution process (default 60s); A timeout error occurs if it fails to complete within the period.")
