...
func (j *CommandUnregisterOption) Run() error {
	...
	karmadaConfigSecret, err := j.MemberClusterClient.CoreV1().Secrets(j.Namespace).Get(context.TODO(), register.KarmadaConfigSecretName, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("failed to get the secret which stores the karmada kubeconfig")
	}
	karmadaCfg, err := clientcmd.Load(karmadaConfigSecret.Data[register.KarmadaConfigSecretKey])
	if err != nil {
		return err
	}
...
func (j *CommandUnregisterOption) RunUnregisterCluster() error {
	...
	clusterResources := []obj{
		...
		{Kind: "Secret", Namespace: j.Namespace, Name: register.KarmadaConfigSecretName},
	...
	localObsoleteFiles := []obj{
		{Kind: "File", Name: filepath.Join(register.KarmadaDir, register.KarmadaAgentBootstrapKubeConfigFileName)},
