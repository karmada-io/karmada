...
// CalcAvailableReplicas get the minimum value of MaxAvailableReplicas in terms of all estimators.
func CalcAvailableReplicas(clusters []*clusterv1alpha1.Cluster, spec *workv1alpha2.ResourceBindingSpec) []workv1alpha2.TargetCluster {
	availableTargetClusters := make([]workv1alpha2.TargetCluster, len(clusters))
	...
	estimators := GetReplicaEstimators()
	...
	for _, estimator := range estimators {
		...
		for i := range res {
			if res[i].Replicas == UnauthenticReplica {
				continue
			}
