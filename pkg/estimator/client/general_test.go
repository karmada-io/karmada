...
func TestGetMaximumReplicasBasedOnResourceModels(t *testing.T) {
	...
	for _, tt := range tests {
		pinedCase := tt // only for Go 1.21 or lower.
		t.Run(pinedCase.name, func(t *testing.T) {
			replicas, err := getMaximumReplicasBasedOnResourceModels(&pinedCase.cluster, &pinedCase.replicaRequirements)
			if pinedCase.expectError && err == nil {
			...
			if replicas != pinedCase.expectedReplicas {
				t.Errorf("getMaximumReplicasBasedOnResourceModels() = %v, expectedReplicas %v", replicas, pinedCase.expectedReplicas)
			}
		})
	}
}
