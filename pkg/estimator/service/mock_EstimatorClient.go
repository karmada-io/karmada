...
// GetVictimResourceBindings provides a mock function with given fields: ctx, in, opts
func (_m *MockEstimatorClient) GetVictimResourceBindings(ctx context.Context, in *pb.PreemptionRequest, opts ...grpc.CallOption) (*pb.PreemptionResponse, error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	...
	var r0 *pb.PreemptionResponse
	if rf, ok := ret.Get(0).(func(context.Context, *pb.PreemptionRequest, ...grpc.CallOption) *pb.PreemptionResponse); ok {
		r0 = rf(ctx, in, opts...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*pb.PreemptionResponse)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, *pb.PreemptionRequest, ...grpc.CallOption) error); ok {
		r1 = rf(ctx, in, opts...)
	} else {
		r1 = ret.Error(1)
	}
