...
func (pl *TestPlugin) EstimateComponents(_ context.Context, _ *schedcache.Snapshot, _ []*pb.Component, _ string, _ []*pb.AssumedWorkload) (int32, *framework.Result) {
...
func Test_frameworkImpl_RunEstimateComponentsPlugins(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sets, ret := f.RunEstimateComponentsPlugins(ctx, nil, []*pb.Component{}, "", nil)
