...
func (frw *frameworkImpl) RunEstimateComponentsPlugins(ctx context.Context, snapshot *schedcache.Snapshot, components []*pb.Component, namespace string, assumedWorkloads []*pb.AssumedWorkload) (int32, *framework.Result) {
	startTime := time.Now()
	defer func() {
	...
	for _, pl := range frw.estimateComponentsPlugins {
		plSets, ret := frw.runEstimateComponentsPlugins(ctx, pl, snapshot, components, namespace, assumedWorkloads)
...
func (frw *frameworkImpl) runEstimateComponentsPlugins(ctx context.Context, pl framework.EstimateComponentsPlugin, snapshot *schedcache.Snapshot, components []*pb.Component, namespace string, assumedWorkloads []*pb.AssumedWorkload) (int32, *framework.Result) {
	startTime := time.Now()
	sets, ret := pl.EstimateComponents(ctx, snapshot, components, namespace, assumedWorkloads)
