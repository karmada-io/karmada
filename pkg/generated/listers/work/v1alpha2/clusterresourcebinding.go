...
type ClusterResourceBindingLister interface {
	...
	List(selector labels.Selector) (ret []*workv1alpha2.ClusterResourceBinding, err error)
	// Get retrieves the ClusterResourceBinding from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*workv1alpha2.ClusterResourceBinding, error)
...
type clusterResourceBindingLister struct {
	listers.ResourceIndexer[*workv1alpha2.ClusterResourceBinding]
}
...
func NewClusterResourceBindingLister(indexer cache.Indexer) ClusterResourceBindingLister {
	return &clusterResourceBindingLister{listers.New[*workv1alpha2.ClusterResourceBinding](indexer, workv1alpha2.Resource("clusterresourcebinding"))}
}
