...
func (d *ResourceDetector) propagateResource(object *unstructured.Unstructured,
	...
	// if so, just apply it.
	claimedID, idExist := object.GetLabels()[policyv1alpha1.PropagationPolicyPermanentIDLabel]
	if idExist {
		claimedNamespace := object.GetAnnotations()[policyv1alpha1.PropagationPolicyNamespaceAnnotation]
		claimedName := object.GetAnnotations()[policyv1alpha1.PropagationPolicyNameAnnotation]
		return d.getAndApplyPolicy(object, objectKey, resourceChangeByKarmada, claimedID, claimedNamespace, claimedName)
	}

	// 2. Check if the object has been claimed by a ClusterPropagationPolicy,
	// if so, just apply it.
	claimedID, idExist = object.GetLabels()[policyv1alpha1.ClusterPropagationPolicyPermanentIDLabel]
	if idExist {
		claimedName := object.GetAnnotations()[policyv1alpha1.ClusterPropagationPolicyAnnotation]
		return d.getAndApplyClusterPolicy(object, objectKey, resourceChangeByKarmada, claimedID, claimedName)
	}
...
func (d *ResourceDetector) getAndApplyPolicy(object *unstructured.Unstructured, objectKey keys.ClusterWideKey,
	resourceChangeByKarmada bool, claimedID, policyNamespace, policyName string) error {
...
func (d *ResourceDetector) getAndApplyClusterPolicy(object *unstructured.Unstructured, objectKey keys.ClusterWideKey,
	resourceChangeByKarmada bool, policyID, policyName string) error {
...
func (d *ResourceDetector) cleanUnmatchedCRBs(policyID, policyName string, selectors []policyv1alpha1.ResourceSelector) error {
	bindings, err := d.listCPPDerivedCRB(policyID, policyName)
	if err != nil {
		return err
	}
...
func (d *ResourceDetector) removeRBsMarks(bindings *workv1alpha2.ResourceBindingList, selectors []policyv1alpha1.ResourceSelector, labels, annotations []string) error {
	...
	for _, binding := range bindings.Items {
		removed, err := d.removeResourceMarksIfMatched(binding.Spec.Resource, selectors, labels, annotations)
		if err != nil {
...
func (d *ResourceDetector) removeCRBsMarks(bindings *workv1alpha2.ClusterResourceBindingList,
	...
	var errs []error
	for _, binding := range bindings.Items {
		removed, err := d.removeResourceMarksIfMatched(binding.Spec.Resource, selectors, removeLabels, removeAnnotations)
		if err != nil {
			klog.Errorf("Failed to remove resource labels when resource not match with policy selectors, err: %v", err)
			errs = append(errs, err)
			continue
...
func (d *ResourceDetector) removeResourceMarksIfMatched(objectReference workv1alpha2.ObjectReference,
...
func (d *ResourceDetector) listPPDerivedRBs(policyID, policyNamespace, policyName string) (*workv1alpha2.ResourceBindingList, error) {
	...
	listOpt := &client.ListOptions{
		...
		LabelSelector: labels.SelectorFromSet(labels.Set{policyv1alpha1.PropagationPolicyPermanentIDLabel: policyID}),
	}
