...
func (d *ResourceDetector) HandlePropagationPolicyCreationOrUpdate(policy *policyv1alpha1.PropagationPolicy) error {
	...
	for _, rb := range resourceBindings.Items {
		...
		d.enqueueResourceKeyWithActivationPref(resourceKey, policy.Spec.ActivationPreference)
	}
	...
	for _, key := range matchedKeys {
		d.RemoveWaiting(key)
		d.enqueueResourceKeyWithActivationPref(key, policy.Spec.ActivationPreference)
	}
...
func (d *ResourceDetector) enqueueResourceKeyWithActivationPref(key keys.ClusterWideKey, pref policyv1alpha1.ActivationPreference) {
