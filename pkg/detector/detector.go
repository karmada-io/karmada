...
func (d *ResourceDetector) handlePolicyDeletion(claimMetadata labels.Set, resources []policyv1alpha1.ResourceSelector, cleanupFunc func(obj metav1.Object)) error {
	...
	for _, resource := range util.ExtractUniqueNamespacedSelectors(resources) {
		...
		if apierrors.IsNotFound(err) {
			klog.Infof("Skip cleanup as the server could not find the requested resource(%s, kind=%s, namespace=%s). This may be because a namespace was incorrectly specified for a cluster-scoped resource.", objRef.APIVersion, objRef.Kind, objRef.Namespace)
			continue
		}
		if err != nil {
