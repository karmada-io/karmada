...
// UpdatePlacement update the cluster name list when some clusters are already scheduled.
func UpdatePlacement(scheduled ScheduleResult, placement v1alpha1.Placement) []string {
   clusterNames := placement.ClusterAffinity.ClusterNames
   for i := 0; i < len(clusterNames); i++ {
      for _, name := range scheduled.SuggestedClusters {
   ...
   klog.V(3).Infof("The number of current ")
   return clusterNames
}


func (g *genericScheduler) rescheduler(cluster *v1.Condition, ) []string {
...
func (g *genericScheduler) rescheduler() {

   klog.V(3).Infof("Try to detect cluster health status")

   var scheduledClusters ScheduleResult
   numOfScheduledClusters := len(scheduledClusters.SuggestedClusters)

   for i := range scheduledClusters.SuggestedClusters {

   }
   var numOfNotReadyClusters int

   //numOfScheduledClusters :=  len([]v1alpha1.TargetCluster) //TODO
   //TODO: update placement

   for k := 0; k < numOfScheduledClusters; k++ {
      if v1.Condition.Reason == "ClusterNotReady" {
         numOfNotReadyClusters += 1

      }
   }

   if numOfScheduledClusters !=0 {
      return nil
   }

   out := make([]string, numOfNotReadyClusters)
   for i := 0; i < numOfNotReadyClusters; i++ {
...
func (g *genericScheduler) ForeverRescheduler() error {
   go wait.Until(g.rescheduler, 5*time.Minute, wait.NeverStop)
   return nil
}
```